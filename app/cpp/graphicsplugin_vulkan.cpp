...
// ShaderProgram to hold a pair of vertex & fragment cpp.shaders
struct ShaderProgram {
  ...
  void LoadVertexShader(const std::string &code) { Load(0, code); }

  void LoadFragmentShader(const std::string &code) { Load(1, code); }
 ...
 private:
  ...
  void Load(uint32_t index, const std::string &code) {
    ...
    switch (index) {
      case 0: si.stage = VK_SHADER_STAGE_VERTEX_BIT;
        name = "vertex";
        break;
      case 1: si.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
        name = "fragment";
        break;
      default: THROW(Fmt("Unknown code index %d", index));
    }

    modInfo.codeSize = code.size() * sizeof(code[0]);
    modInfo.pCode = reinterpret_cast<const uint32_t *>(&code[0]);
    ...
    utils::logger::Log(utils::logger::Level::INFO, Fmt("Loaded %s shader", name.c_str()));
  }
};
...
struct VulkanGraphicsPlugin : public IGraphicsPlugin {
  VulkanGraphicsPlugin(const std::shared_ptr<Options> & /*unused*/,
  ...
  void InitializeDevice(XrInstance instance, XrSystemId systemId) override {
    ...
    vkCreateDebugReportCallbackEXT =
        (PFN_vkCreateDebugReportCallbackEXT) vkGetInstanceProcAddr(m_vkInstance,
                                                                   "vkCreateDebugReportCallbackEXT");
    vkDestroyDebugReportCallbackEXT =
        (PFN_vkDestroyDebugReportCallbackEXT) vkGetInstanceProcAddr(m_vkInstance,
                                                                    "vkDestroyDebugReportCallbackEXT");
    ...
    CHECK_VKCMD(vkCreateDebugReportCallbackEXT(m_vkInstance, &debugInfo, nullptr,
                                               &m_vkDebugReporter));
  ...
  void InitializeResources() {
    if (vert_shader.empty()) THROW("Failed to compile vertex shader");
    if (frag_shader.empty()) THROW("Failed to compile fragment shader");

    m_shaderProgram.Init(m_vkDevice);
    m_shaderProgram.LoadVertexShader(vert_shader);
    m_shaderProgram.LoadFragmentShader(frag_shader);
 ...
 protected:
  ...
  VkBool32 debugReport(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType,
    ...
    if ((flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) != 0u) {
      flagNames += "ERROR:";
      level = utils::logger::Level::FATAL;
    }
...
#define MK_OBJECT_TYPE_CASE(name)                  \
    ...
    utils::logger::Log(level,
                       Fmt("%s (%s 0x%llx) [%s] %s", flagNames.c_str(), objName.c_str(), object,
  ...
  debugReportThunk(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType,
                   uint64_t object, size_t location, int32_t messageCode,
                   const char *pLayerPrefix, const char *pMessage, void *pUserData) {
    return static_cast<VulkanGraphicsPlugin *>(pUserData)->debugReport(flags, objectType,
                                                                       object, location,
                                                                       messageCode,
                                                                       pLayerPrefix,
                                                                       pMessage);
  ...
  virtual XrStructureType
  GetGraphicsBindingType() const { return XR_TYPE_GRAPHICS_BINDING_VULKAN2_KHR; }

  virtual XrStructureType
  GetSwapchainImageType() const { return XR_TYPE_SWAPCHAIN_IMAGE_VULKAN2_KHR; }
...
struct VulkanGraphicsPluginLegacy : public VulkanGraphicsPlugin {
  ...
  virtual XrStructureType
  GetGraphicsBindingType() const override { return XR_TYPE_GRAPHICS_BINDING_VULKAN_KHR; }

  virtual XrStructureType
  GetSwapchainImageType() const override { return XR_TYPE_SWAPCHAIN_IMAGE_VULKAN_KHR; }
  ...
  virtual XrResult CreateVulkanInstanceKHR(XrInstance instance,
    ...
    CHECK_XRCMD(pfnGetVulkanInstanceExtensionsKHR(instance, createInfo->systemId, 0,
                                                  &extensionNamesSize, nullptr));

    std::vector<char> extensionNames(extensionNamesSize);
    CHECK_XRCMD(pfnGetVulkanInstanceExtensionsKHR(instance, createInfo->systemId,
                                                  ...
                                                  &extensionNames[0]));
    {
      ...
      auto pfnCreateInstance = (PFN_vkCreateInstance) createInfo->pfnGetInstanceProcAddr(
  ...
  CreateVulkanDeviceKHR(XrInstance instance, const XrVulkanDeviceCreateInfoKHR *createInfo,
    ...
    CHECK_XRCMD(pfnGetVulkanDeviceExtensionsKHR(instance, createInfo->systemId, 0,
                                                &deviceExtensionNamesSize, nullptr));
    std::vector<char> deviceExtensionNames(deviceExtensionNamesSize);
    CHECK_XRCMD(pfnGetVulkanDeviceExtensionsKHR(instance, createInfo->systemId,
                                                deviceExtensionNamesSize,
                                                &deviceExtensionNamesSize,
                                                &deviceExtensionNames[0]));
    {
      // Note: This cannot outlive the extensionNames above, since it's just a collection of views into that string!
      std::vector<const char *> extensions = ParseExtensionString(
          &deviceExtensionNames[0]);
      ...
      deviceInfo.ppEnabledExtensionNames = extensions.empty() ? nullptr
                                                              : extensions.data();

      auto pfnCreateDevice = (PFN_vkCreateDevice) createInfo->pfnGetInstanceProcAddr(
          m_vkInstance, "vkCreateDevice");
      *vulkanResult = pfnCreateDevice(m_vkPhysicalDevice, &deviceInfo,
